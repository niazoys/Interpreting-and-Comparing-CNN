...
class compute_iou:
    ...
    def IoU(self,activation,concept_map,unit,label,concept_number):
        ...
        '''
        self.intersection_score[unit,label,concept_number]+=np.sum(np.logical_and(activation,concept_map))
        self.union_score[unit,label,concept_number]+=np.sum(np.logical_or(activation,concept_map))
