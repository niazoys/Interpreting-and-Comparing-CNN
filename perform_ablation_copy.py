...
def ablate(model,selected_class,Top,percentile):
    ...
    print(np.asarray(top_dict))
    if Top:
        np.save('top_bottom_index/resnet18/top_class_0'+str(selected_class)+'.npy',np.asarray(top_dict))
    else:
        np.save('top_bottom_index/resnet18/bottom_class_0'+str(selected_class)+'.npy',np.asarray(bottom_dict))
